...
impl Instruction {
    ...
    pub fn to_asm(&self) -> String {
        ...
        match self {
            ...
            IfEq(vx, byte) => format!("se v{} 0x{:02X}", vx, byte),
            IfNeq(vx, byte) => format!("sne v{} 0x{:02X}", vx, byte),
            IfEqRg(vx, vy) => format!("se v{} v{}", vx, vy),
            Set(vx, byte) => format!("ld v{} 0x{:02X}", vx, byte),
            Add(vx, val) => format!("add v{} 0x{:02X}", vx, val),
            SetRg(vx, vy) => format!("ld v{} v{}", vx, vy),
            Or(vx, vy) => format!("or v{} 0x{:02X}", vx, vy),
            And(vx, vy) => format!("and v{} v{}", vx, vy),
            Xor(vx, vy) => format!("xor v{} v{}", vx, vy),
            AddRg(vx, vy) => format!("add v{} v{}", vx, vy),
            Sub(vx, vy) => format!("sub v{} 0x{:02X}", vx, vy),
            RightShift(vx) => format!("shr v{}", vx),
            SubSelf(vx, vy) => format!("subn v{} v{}", vx, vy),
            LeftShift(vx) => format!("shl v{}", vx),
            IfNeqRg(vx, vy) => format!("sne v{} v{}", vx, vy),
            SetAddr(addr) => format!("ld i 0x{:02X}", addr),
            Jump(addr) => format!("jp 0x{:2X}", addr),
            Rand(vx, byte) => format!("rnd v{} 0x{:02X}", vx, byte),
            Disp(vx, vy, nibble) => format!("drw v{} v{} 0x{:02X}", vx, vy, nibble),
            KeyOpEq(vx) => format!("skp v{}", vx),
            KeyOpNeq(vx) => format!("sknp v{}", vx),
            GetTimer(vx) => format!("ld v{} dt", vx),
            GetKeyOp(vx) => format!("ld v{} k", vx),
            SetTimer(vx) => format!("ld dt v{}", vx),
            SetSoundTimer(vx) => format!("ld st v{}", vx),
            AddToI(vx) => format!("add i v{}", vx),
            FontLoad(vx) => format!("ld f v{}", vx),
            BCD(vx) => format!("ld b v{}", vx),
            MemDump(vx) => format!("ld [i] v{}", vx),
            MemLoad(vx) => format!("ld v{} [i]", vx),
